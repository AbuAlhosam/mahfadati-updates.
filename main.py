...
ASSETS_DIR = os.environ.get("FLET_ASSETS_DIR", os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets"))
...
LICENSE_SECRET = "o5gdyGd6SjeYowz1mXRSuwWH21tU" 
...
def main(page: ft.Page): 
    ...
    about_overlay = ft.Container( 
        visible=False, bgcolor="#000000CC", expand=True, 
        alignment=ft.Alignment(0, -0.8), top=0, left=0, right=0, bottom=0, 
        content=ft.Container( 
            width=300, padding=20, border_radius=12, bgcolor="#1A2635", 
            content=ft.Column(tight=True, spacing=10, 
                horizontal_alignment=ft.CrossAxisAlignment.CENTER, 
                controls=[ 
                    ...
                    ft.ElevatedButton( 
                        content=ft.Text("💬 تواصل عبر واتساب", color="#0F1923", weight=ft.FontWeight.BOLD, size=13), 
                        bgcolor="#25D366", on_click=lambda _: open_whatsapp(None)), 
    ...
    def do_export(report_type):
        ...
        try:
            ...
            show_info(f"تم حفظ التقرير في:\n{fname}")
        except Exception as e:
            show_info(f"❌ فشل إنشاء التقرير: {e}")
            ...
            content=ft.Row(alignment=ft.MainAxisAlignment.SPACE_BETWEEN, controls=[ 
                ...
                ft.Row(spacing=4, controls=[ 
                    ft.Text( 
                        ...
                        tooltip=( 
                            f"باقي {days_left[0]} يوم من التجربة المجانية.\nاشترك مدى الحياة للاستمرار بكل الميزات." 
                            if (trial_ok[0] and not premium_ok[0]) 
                            else ("أنت مشترك مدى الحياة ✅" if premium_ok[0] else "انتهت فترة التجربة، اشترك للاستمرار") 
                        ), 
                    ), 
